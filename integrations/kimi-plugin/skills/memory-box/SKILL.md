...
Baidu folder mode uses an already synchronized local folder and must not request passwords/cookies. Cloud-folder and LAN sync are E2EE in v0.9. Before cloud sync, use `memory_device_identity` and `memory_sync_devices`, compare full fingerprints on both machines, and mutually trust the devices with `memory_sync_trust_device`.
